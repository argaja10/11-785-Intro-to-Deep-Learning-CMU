    ...
    def forward(self, x, h_0=None):
        ...
        for i in range(seq_len):
            ...
            for j in range(len(self.rnn)):
                l=len(self.hiddens)
                ht_i = self.rnn[j].forward(ht,self.hiddens[l-1][j])
                ht = ht_i
                hidden.append(ht_i)
            self.hiddens.append(hidden.copy())
        logits = self.output_layer.forward(ht)
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in range(seq_len-1,-1,-1):
            for i in range(self.num_layers-1, -1, -1):
                ...
                dx_back, dh_back = self.rnn[i].backward(dh[i],self.hiddens[t+1][i],h_prev_l,h_prev_t)
                dh[i] = dh_back
                if i!=0:
                    dh[i-1] = dh[i-1] + dx_back
