...
class MLP(object):
    ...
    def __init__(self, input_size, output_size, hiddens, activations, weight_init_fn,
        ...
        self.shape = [self.input_size] + hiddens + [self.output_size]
        self.linear_layers = [Linear(in_feature, out_feature, weight_init_fn, bias_init_fn) for in_feature, out_feature in zip(self.shape[:-1], self.shape[1:])]

        # If batch norm, add batch norm layers into the list 'self.bn_layers'
        if self.bn:
            self.bn_layers = []
            for i in range(1, self.num_bn_layers+1):
                self.bn_layers.append(BatchNorm(self.shape[i]))
    ...
    def forward(self, x):
        ...
        for i in range(self.nlayers):
            y_out = self.linear_layers[i].forward(y_in)
            if i in range(self.num_bn_layers):
                if self.train_mode:
                    y_out = self.bn_layers[i].forward(y_out)
                else:
                    y_out = self.bn_layers[i].forward(y_out, eval=True)

            y_in = self.activations[i].forward(y_out)
        self.inputs = y_in    
        return y_in    
    ...
    def backward(self, labels):
        ...
        input_deriv = []
        output_deriv = [derivative]
        
        input_bn_deriv = []
        for i in range(self.nlayers-1,-1,-1):
            if self.nlayers-1-i<self.nlayers-self.num_bn_layers:
                input_deriv.append(output_deriv[self.nlayers-1-i]*self.activations[i].derivative())
                output_deriv.append(self.linear_layers[i].backward(input_deriv[self.nlayers-1-i]))
            else:
                input_bn_deriv.append(output_deriv[self.nlayers-1-i]*self.activations[i].derivative())
                input_deriv.append(self.bn_layers[i].backward(input_bn_deriv[self.num_bn_layers-i-1]))
                output_deriv.append(self.linear_layers[i].backward(input_deriv[self.nlayers-1-i]))
